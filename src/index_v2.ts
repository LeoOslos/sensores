...
function calcularHumedadAbsoluta(temp: number, hr: number): number {
  const e = (hr / 100) * 6.112 * Math.exp((17.67 * temp) / (temp + 243.5));
  return parseFloat(((e * 216.7) / (temp + 273.15)).toFixed(2));
}
...
function debeEnviarAlerta(sensor: string): boolean {
  ...
  if (horaActual < 10 || horaActual >= 22) return false;
  const alertas = leerAlertasEnviadas();
...
async function enviarAlerta(sensor: string, estadoBateria: string) {
  ...
  if (!debeEnviarAlerta(sensor)) return;
...
// Paso 17: Agregamos "headerId: true" para que escriba los nombres de columna
const CSV_PATH = 'devices.csv';
...
function getFechaBuenos_Aires() {
  ...
  const fecha = buenosAires.toLocaleDateString('es-AR', { year: 'numeric', month: '2-digit', day: '2-digit' });
  const hora = buenosAires.toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false });
  return { fecha, hora };
}

function sha256(content: string) { return CryptoJS.SHA256(content).toString(CryptoJS.enc.Hex); }
function hmac(str: string) { return CryptoJS.HmacSHA256(str, CLIENT_SECRET).toString(CryptoJS.enc.Hex).toUpperCase(); }

function buildSign(method: string, path: string, token: string, t: string, body: string = '') {
...
function parsearDatos(status: any[]) {
  ...
  for (const item of status) {
    switch (item.code) {
      case 'va_temperature': datos.temperatura = item.value / 10; break;
      case 'va_humidity': datos.humedad = item.value; break;
      case 'battery_state':
        datos.bateria_raw = item.value;
        datos.bateria = BATTERY_STATES[item.value] || item.value;
        break;
      case 'temp_unit_convert': datos.unidad_temp = item.value.toUpperCase(); break;
      default:
...
async function main() {
  try {
    ...
    const idBalcon = process.env.TUYA_DEVICE_ID_BALCONY || '';
    const idInterior = process.env.TUYA_DEVICE_ID_INTERIOR || '';

    let dataBalcon: any = null;
    let dataInterior: any = null;

    console.log('\n--- Registro de sensores (Arquitectura Limpia) ---');

    for (const d of [idBalcon, idInterior]) {
      if (!d) continue;
      const status = await getDeviceStatus(token, d);
      const datos = parsearDatos(status);
      const vpd = calcularVPD(datos.temperatura, datos.humedad);

      if (d === idBalcon) dataBalcon = { ...datos, vpd };
      else dataInterior = { ...datos, vpd };
      ...
      const row = {
        ...
        unidad_temp: datos.unidad_temp ?? 'C',
      ...
      console.log(`${row.sensor} -> T: ${row.temperatura}°C, H: ${row.humedad}%, VPD: ${vpd} kPa`);
    }

    if (dataBalcon && dataInterior) {
      console.log("\n--- Sugerencias del Sistema ---");
      const haBalcon = calcularHumedadAbsoluta(dataBalcon.temperatura, dataBalcon.humedad);
      const haInterior = calcularHumedadAbsoluta(dataInterior.temperatura, dataInterior.humedad);
      
      if (dataBalcon.temperatura < dataInterior.temperatura && haBalcon < haInterior) {
        console.log("💡 SUGERENCIA: Abrí las ventanas.");
      } else {
        console.log("💡 SUGERENCIA: Mantené cerrado.");
      }
    }
  } catch (err) {
